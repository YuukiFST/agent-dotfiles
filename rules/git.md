...
The first line must **not** attribute authorship to tools or assistants — no "made with", "generated by", `por/com/by/with/via <tool>`, `agente`, `IA`. Naming a tool as the **object** of the change is fine when the code genuinely concerns it (`fix openai client`, `update gemini adapter`, `feat(skills): extend autoreview to Cursor and Pi`).
