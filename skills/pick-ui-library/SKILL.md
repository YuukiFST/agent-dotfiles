...
**Decorative effects (Jakub Antalik set).** Three real, published, zero-to-few-dep **React** packages — install them, don't hand-roll or paste frozen shader/CSS. **React-only prerequisite:** all three ship as React components (React 18+), so first confirm React is in `package.json`; on a non-React project skip them entirely and say so rather than substituting a look-alike (`metal-fx` additionally needs WebGL support in the browser). They are *flourish*, not structure: one hero moment per view, never a page full of glowing metal. The reusable product-motion catalog (modals, dropdowns, toasts, accordions, 27 snippets) lives in the separate `transitions-dev` skill — reach there first for functional motion, here only for decorative shine.
