...
**Decorative effects (Jakub Antalik set).** Two real, published, zero-to-few-dep **React** packages — install them, don't hand-roll or paste frozen CSS. **React-only prerequisite:** both ship as React components (React 18+), so first confirm React is in `package.json`; on a non-React project skip them entirely and say so rather than substituting a look-alike. They are *flourish*, not structure: one hero moment per view, never a page full of glow. The reusable product-motion catalog (modals, dropdowns, toasts, accordions, 27 snippets) lives in the separate `transitions-dev` skill — reach there first for functional motion, here only for decorative shine.
...
- **A hand-CSS glowing/animated border or a custom AI "thinking" spinner** → border-beam / thinking-orbs already do these (Decorative effects above).
