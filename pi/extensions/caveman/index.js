...
import { getCavemanInstructions } from "./caveman-instructions.js";

const RUNTIME_MODE_LIST = RUNTIME_MODES.join("|");

function resolveSessionMode(entries, fallbackMode = DEFAULT_MODE) {
...
function parseCavemanCommand(args, defaultMode = DEFAULT_MODE) {
  const fallback = normalizeMode(defaultMode) || DEFAULT_MODE;
  const normalizedText = String(args || "").trim().toLowerCase();
...
export default function cavemanExtension(pi) {
  ...
  function syncStatus(ctx) {
    ...
    c.ui.setStatus(
      "caveman",
      `${indicator} 🪨 ${theme.fg("muted", "caveman: ")}${theme.fg("text", currentMode.toUpperCase())}`,
    );
  }
  ...
  pi.registerCommand("caveman", {
    description: `Set mode: ${RUNTIME_MODE_LIST}. Commands: status, default <mode>`,
  ...
  pi.on("before_agent_start", async (event) => {
    ...
    return { systemPrompt: `${base}${getCavemanInstructions(currentMode)}` };
  });
}
