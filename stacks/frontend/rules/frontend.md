...
3. `interface-review` — the cross-discipline gate over the **change**, not the screen: `/interface-review` (uncommitted + branch by default), or `interface-review quick`, `interface-review pr <n>`, a branch, a ref, a range. It resolves the scope, expands each changed file one hop to the surfaces that render it (two hops for tokens and shared primitives), reads the `-` side of every hunk for removed a11y/focus/motion/text signals, and statuses every finding `Introduced` / `Regression` / `Pre-existing` — then hands the review to `better-interface`, which routes the six domain skills (`better-accessibility`, `better-layout`, `better-writing`, `better-typography`, `better-colors`, `better-ui`), consolidates by root cause and issues one verdict: `Block` (any HIGH) / `Needs changes` / `Approve`. Caps: 5 findings in `quick` (HIGH+MEDIUM only), 15 in `full`; pre-existing findings sit outside cap and verdict, so touching a legacy file never becomes a full-file audit. Read-only, including the checkout — it fetches PR refs, never `gh pr checkout`. Interface quality only: correctness, tests and security stay with `autoreview` / `security-review`. **No diff to review** (auditing a screen or flow that is already committed) → call `better-interface` directly with the screen as scope. Re-run in `quick` mode if the browser pass below forces UI fixes.
4. **Browser pass** — harness browser pane, `agent-browser` (`~/.agents/skills/agent-browser/SKILL.md`), or `webapp-testing` (archived in `stacks/webapp-testing/`; enable it first): load the real page, screenshot, test interactions, check console errors, verify mobile width + dark mode. Iterate screenshot → fix → screenshot; done only on a clean pass.
5. **Lighthouse on any page a user actually loads** (landing, marketing, public product page): `npx lighthouse@latest <url> --quiet --chrome-flags="--headless" --output=json --output-path=stdout`. Report the four scores; a red Performance or Accessibility score is a bug, not a nice-to-have. Skip for internal CRUD behind auth.
