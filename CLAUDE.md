...
- **Frontend/UI work — above all a /goal building from a PRD that includes a front** → `~/.claude/rules/frontend.md` (skill pipeline, phases, minimum bar). MANDATORY before UI code. Frontend pipeline skills are router-only (`disable-model-invocation`); read them from `~/.agents/skills/<name>/SKILL.md` per step — never rely on the skill catalog. SKIP the file entirely when the project has its own Design System (follow the DS exactly) or the change is a trivial fix ≤10 lines.
